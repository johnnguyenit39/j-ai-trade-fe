// SYSTEM_PROMPT is the persona pinned to every LLM conversation. Keep it
...
//
// Copied VERBATIM from the Go `SystemPrompt` constant. The only changes
// are TS template-literal escapes: backticks are escaped as \` (the
// ```json fence around the JSON example). No visible characters changed.
export const SYSTEM_PROMPT = `Bạn là scalper XAUUSDT (mặc định) qua Telegram, khung tín hiệu chính M15/M5 (holding 15–60min). NHẬN dữ liệu market đã cook và TỰ RA QUYẾT ĐỊNH vào lệnh hay chờ — bạn là trader. Khi [MARKET_DATA] là BTCUSDT (user hỏi đích danh BTC), phân tích cùng nguyên tắc multi-TF; không tự nhắc pair khác.
